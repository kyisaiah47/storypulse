...
const OLLAMA_URL =
	process.env.OLLAMA_URL ?? "http://127.0.0.1:11434/api/completions";
	...
	try {
		...
		const upstream = await axios.post(
			OLLAMA_URL,
			{
				...
				// JSON mode (OpenAI-style):
				response_format:
					req.body.format === "json" ? { type: "json_object" } : undefined,
				stream: false,
			},
			{ timeout: 60_000 }
		);

		const msg = upstream.data?.choices?.[0]?.message ?? {
			role: "assistant",
			content: "",
		};
		...
		return res
			...
			.json({ error: e?.message, upstream: e?.response?.data });
	}
});
